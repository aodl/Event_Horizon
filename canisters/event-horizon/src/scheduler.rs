use std::{cell::{Cell, RefCell}, time::Duration};

use crate::{cadence::{next_mode, PollingMode}, config, funding, logging, polling, state};
...
fn schedule_poll(delay: Duration) {
    let timer_id = ic_cdk_timers::set_timer(delay, async {
        POLL_TIMER.with_borrow_mut(|slot| { slot.take(); });
        let acquired = POLL_RUNNING.with(|flag| {
            if flag.get() { false } else { flag.set(true); true }
        });
        if !acquired {
    ...
    POLL_TIMER.with_borrow_mut(|slot| {
        if let Some(previous) = slot.replace(timer_id) { ic_cdk_timers::clear_timer(previous); }
    });
}

fn schedule_funding(delay: Duration) {
    let timer_id = ic_cdk_timers::set_timer(delay, async {
        FUNDING_TIMER.with_borrow_mut(|slot| { slot.take(); });
...
pub fn start() {
    ...
    let mut meta = state::read_metadata();
    let mode = next_mode(meta.polling_mode, balance);
    if mode != meta.polling_mode { meta.polling_mode = mode; state::write_metadata(meta); }
    logging::reserve_mode(mode == PollingMode::ReserveProtection, balance);
...
pub async fn debug_poll_once() { polling::run_poll().await; }

#[cfg(feature = "debug_api")]
pub async fn debug_funding_once() { funding::run_funding_maintenance().await; }
