...
struct NotifyTopUpArg { canister_id: Principal, block_index: u64 }

#[derive(Clone, Debug, CandidType, Deserialize, PartialEq, Eq)]
enum NotifyTopUpResult { Ok(Nat), Err(NotifyError) }
...
pub async fn notify_top_up(cmc: Principal, canister_id: Principal, block_index: u64) -> NotifyOutcome {
    let response = match Call::bounded_wait(cmc, "notify_top_up")
        .with_arg(&NotifyTopUpArg { canister_id, block_index })
        .await
    {
