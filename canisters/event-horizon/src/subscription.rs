...
use crate::{account::{account_identifier_bytes, numbered_subaccount}, memo::SubscriptionDeclaration};
...
impl Subscription {
    pub fn account_identifier(&self) -> [u8; 32] {
        account_identifier_bytes(self.subscriber, numbered_subaccount(self.numbered_subaccount))
    }
    ...
    match existing {
        Some(existing)
            if existing.subscriber == candidate.subscriber
                ...
                && existing.minimum_e8s <= candidate.minimum_e8s => existing,
        _ => candidate,
    }
}
...
mod tests {
    ...
    fn p() -> Principal { Principal::from_text("r5m5y-diaaa-aaaaa-qanaa-cai").unwrap() }

    #[test]
    fn lower_threshold_replaces_higher_for_same_account() {
        let high = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 100_000_000 };
        let low = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 1_000_000 };
        assert_eq!(merge_subscription(Some(high), low.clone()), low);
    }

    #[test]
    fn unfiltered_subscription_subsumes_thresholded_subscription() {
        let thresholded = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 1_000_000 };
        let unfiltered = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 0 };
        assert_eq!(merge_subscription(Some(thresholded), unfiltered.clone()), unfiltered);
    }
    ...
    fn threshold_is_inclusive() {
        let sub = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 1_000_000 };
    ...
    fn omitted_threshold_matches_every_transfer_amount() {
        let sub = Subscription { subscriber: p(), numbered_subaccount: 7, minimum_e8s: 0 };
