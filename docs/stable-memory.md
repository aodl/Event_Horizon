...
Surplus support is additive. IDs 0–6 are never reinterpreted. ID 7 initializes as `Uninitialized`; its first read migrates ID 2 exactly once: old `Idle` becomes new `Idle`, old `TransferPending` becomes `CmcTransferPending` with planned-surplus amount/fee zero and destination/memo absent, and old `NotifyPending` becomes `CmcNotifyPending` with the same absent zero-surplus fields. Thereafter only ID 7 is authoritative and old ID 2 remains untouched historical storage. One cell always determines the next money-moving action, so no crash boundary exists between an authoritative split plan and its execution state.

Every new split freezes the surplus default-account identifier and memo alongside its amount and fee before the retained transfer begins. Those fields survive `CmcTransferPending` to `CmcNotifyPending`; after successful minting and the second health gate, `SurplusTransferPendingV2` contains destination `[u8; 32]`, memo `u64`, amount, fee, and deterministic creation time. Recovery reads no destination or memo from current configuration. Optional destination/memo fields keep the undeployed `4d21a35822d0b9ce04fd097a5e99abb39ccb23d3` FundingStateV2 Candid encoding decodable; post-upgrade hardening freezes its then-current debug destination before calls can resume. The tracked exact fixture proves an uncertain accepted transfer remains bound to that destination after configuration changes.
