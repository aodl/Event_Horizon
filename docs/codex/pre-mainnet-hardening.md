...
- The Rust frontend still embeds and certifies all static assets. It adds the deployment discovery cookie to the document response when the ICP CLI environment value is present. Browser calls use the page origin for a local replica and `https://icp-api.io` on mainnet; the certified CSP narrowly permits the API gateway. Backend `get_pricing` remains the protocol-authoritative display value, and backend admission remains authoritative.
